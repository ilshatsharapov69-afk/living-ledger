...
function must(label: string, actual: number, expected: number) {
  if (actual !== expected) {
    throw new Error(
      `story.ts — ${label} does not add up: got ${actual}, expected ${expected}. ` +
...
for (const doc of [invoices, payroll, operating, tax]) {
  must(`${doc.id} rows`, sum(doc.rows.map((r) => r.amount)), doc.total);
}
must('bank deposits less transfers', sum(bank.rows.filter((r) => !r.excluded).map((r) => r.amount)), bank.total);
must('buckets vs inflow', sum(buckets.map((b) => b.amount)), invoices.total);
must('payroll bucket', buckets[0]!.amount, payroll.total);
must('operating bucket', buckets[1]!.amount, operating.total);
must('tax bucket', buckets[2]!.amount, tax.total);
must('earned', sum(understand.earned.lines.map((l) => l.amount)), understand.earned.answer);
must('cash', sum(understand.cash.lines.map((l) => l.amount)), understand.cash.answer);
must('assets', sum(understand.balance.assets.map((l) => l.amount)), understand.balance.answer);
must('claims', sum(understand.balance.claims.map((l) => l.amount)), understand.balance.answer);
must(
