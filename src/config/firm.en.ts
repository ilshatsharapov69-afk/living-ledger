/**
 * ─────────────────────────────────────────────────────────────────────────────
 *  THE RESCIN FILE — ENGLISH.  This is the only file you touch to make a new
 *  demo. Read the prospect's real site, fill these fields in by hand, build,
 *  send. Target: twenty minutes.
 * ─────────────────────────────────────────────────────────────────────────────
 *
 ...
 *
 * This file also defines the SHAPE every other language is checked against —
 * see the annotation in `firm.ts`. Add a field here and `firm.ru.ts` fails to
 * compile until it has one too, which is the intended order of work.
 *
 * Deliberately no `as const`: two locales are selected between at build time,
 * and a union of two differently-worded literal types is not something
 * `.map()` can be called on.
 */
...
export const firm = {
  ...
  logo: {
    ...
    includesName: false,
    /*
     * The demo mark, drawn rather than typeset, on a 32x32 grid inside a
     * rounded plate. It is the firm's INITIAL, so it belongs to the firm and
     * not to the component: a Russian firm called «Svod» whose logo says N is
     * a demo that has not been localised, it has been translated.
     *
     * Replaced wholesale the moment `src` points at a real file.
     */
    mark: { d: 'M10 23V9.6l12 12.8V9', cap: 'square' as 'square' | 'round' },
  },
   ...
   */
  social: {
    /* Annotated nullable rather than inferred from the placeholder, because
       `null` is a real value here — it is how a platform the firm is not on is
       removed, and the Russian build nulls all three of these. */
    facebook: 'https://www.facebook.com/' as string | null,
    instagram: 'https://www.instagram.com/' as string | null,
    linkedin: 'https://www.linkedin.com/' as string | null,
    /** Where a Russian firm is actually reached. Null here, filled in `firm.ru.ts`. */
   ...
   */
  pricing: {
    /* No currency symbol here: the figure is formatted by `moneyShort`, which
       knows where the symbol goes in each language. */
    period: '/month',
    plans: [
      {
        name: 'Books',
